...
\**************************************************************************)
  ...
  public
    ...
    // FillRoundRect
    function FillRoundRect(ABrush: TGPBrush; ARects: PGPRect; ABorderColor: TGPColor; ARadius: Integer): TStatus;
    ...
    procedure GetRoundRectPath(APath: TGPGraphicsPath; ARect: TGPRect; ADia: Integer);
...
\**************************************************************************)
  ...
  function TGPGraphics.FillRoundRect(ABrush: TGPBrush; ARects: PGPRect; ABorderColor: TGPColor; ARadius: Integer): TStatus;
  var
    FDia  : Integer;
    FPen  : TGPPen;
  ...
  begin
    result       := Ok;
    FDia         := 2 * ARadius;
    FOldPageUnit := Self.GetPageUnit;
    Self.SetPageUnit(UnitPixel);
    FPath        := TGPGraphicsPath.Create(FillModeAlternate);
    FPen         := TGPPen.Create(ABorderColor, 1);
    FPen.SetAlignment(PenAlignmentCenter);

    GetRoundRectPath(FPath, ARects^, FDia);

    Self.FillPath(ABrush, FPath);
    Self.DrawPath(FPen, FPath);

    Self.SetPageUnit(TUnit(FOldPageUnit));
    FPath.Free;
    FPen.Free;
  end;
...
//void FillRoundRect(Graphics* pGraphics, Brush* pBrush, Rect r, Color border, int radius)
{
  ...
  procedure TGPGraphics.GetRoundRectPath(APath: TGPGraphicsPath; ARect: TGPRect; ADia: Integer);
  var
    FCorner : TGPRect;
  begin
    if ADia > ARect.Width  then ADia := ARect.Width;
    if ADia > ARect.Height then ADia := ARect.Height;

    FCorner.X     := ARect.X; FCorner.Y      := ARect.Y;
    FCorner.Width := ADia;    FCorner.Height := ADia;

    APath.Reset;

    // top left
    APath.AddArc(FCorner, 180, 90);

    if (ADia = 20) then
    begin
      FCorner.Width  := FCorner.Width  + 1;
      FCorner.Height := FCorner.Height + 1;
      ARect.Width    := ARect.Width    - 1;
      ARect.Height   := ARect.Height   - 1;
    end;

    // top right
    FCorner.X := FCorner.X + (ARect.Width - ADia - 1);
    APath.AddArc(FCorner, 270, 90);

    // bottom right
    FCorner.Y := FCorner.Y + (ARect.Height - ADia - 1);
    APath.AddArc(FCorner, 0, 90);

    // bottom left
    FCorner.X := FCorner.X - (ARect.Width - ADia - 1);
    APath.AddArc(FCorner, 90, 90);
