      ...
      {                                                                  }
      { date        : 15-02-2002                                         }
      { update date : 02-06-2013 by Coly                                 }
      { - add Initialize functions to TGPRectF record                    }
      {                                                                  }
...
type
  ...
  public
    ...
    procedure Initialize(const ARect: TRect); overload;
    procedure Initialize(const ARect: TGPRectF); overload;
    procedure Add(const ARect: TRect);
    procedure DecHW(const H, W: Integer);
    procedure AddHW(const H, W: Integer);
...
// -----------------------------------------------------------------------------
  ...
  procedure TGPRectF.Initialize(const ARect: TRect);
  begin
    X      := ARect.Left;
    Y      := ARect.Top;
  ...
  procedure TGPRectF.Initialize(const ARect: TGPRectF);
  begin
    X      := ARect.X;
    Y      := ARect.Y;
    Width  := ARect.Width;
  ...
  function TGPRectF.GetTRect: TRect;
  begin
    with Result do
    begin
      Left   := Round(X);
      Top    := Round(Y);
      Right  := Round(X + Width) +1;
      Bottom := Round(Y + Height)+1;
    end;
  end;

  procedure TGPRectF.DecHW(const H, W: Integer);
  begin
    Width  := Width  - W;
    Height := Height - H;
  end;

  procedure TGPRectF.Add(const ARect: TRect);
  begin
    X      := X      + ARect.Right;
    Y      := Y      + ARect.Bottom;
    Width  := Width  + ARect.Right;
    Height := Height + ARect.Bottom;
  end;

  procedure TGPRectF.AddHW(const H, W: Integer);
  begin
    Width  := Width  + W;
    Height := Height + H;
  end;
